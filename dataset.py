...
EXTRA_TIME_DENSE_DIM = 58


class TimeDenseFeaturesComputer:
    """Compute 58-dim time-based dense features per sample.

    Layout: [global(6) | domain_a(12) | domain_b(12) | domain_c(12) | domain_d(12) | cross(4)]
    """

    def __init__(self, seq_domains: List[str]):
    ...
    def compute(
        ...
        seq_data,  # unused, kept for compatibility
        seq_lens: Dict[str, np.ndarray],
        seq_ts: Dict[str, np.ndarray],
    ) -> np.ndarray:
        """Compute time dense features.

        Returns:
            (B, 58) feature matrix: [global(6) | domain0(12) | domain1(12) | ... | cross(4)]
        """
        out = np.zeros((B, EXTRA_TIME_DENSE_DIM), dtype=np.float32)
        offset = 0

        out[:, offset:offset + 6] = self._compute_global_features(timestamps)
        offset += 6

        for domain in self.seq_domains:
            dom_feats = self._compute_domain_features(
                B, timestamps, seq_lens[domain], seq_ts[domain])
            out[:, offset:offset + 12] = dom_feats
            offset += 12

        out[:, offset:offset + 4] = self._compute_cross_domain_features(
            seq_lens, timestamps, seq_ts)
        return out

    def _compute_global_features(self, timestamps: np.ndarray) -> np.ndarray:
        """Compute 6 global time features: hour_sin, hour_cos, dow_sin, dow_cos, is_weekend, is_work_hour."""
        B = timestamps.shape[0]
        hour = (timestamps // 3600) % 24
        dow = ((timestamps // 86400 + 3) % 7).astype(np.float32)
        ...
        is_work_hour = ((hour >= 9) & (hour < 18)).astype(np.float32)

        return np.stack([hour_sin, hour_cos, dow_sin, dow_cos, is_weekend, is_work_hour], axis=1)
    ...
    def _compute_domain_features(
        ...
        """Compute 12 per-domain features."""
        out = np.zeros((B, 12), dtype=np.float32)
        ...
        for i in range(B):
            ...
            has_history = 1.0
            log_seq_len = np.log1p(L)

            last_ts = int(seq_ts[i, L - 1]) if L > 0 else 0
            time_since_last = float(cur_ts[i] - last_ts) if last_ts > 0 else 365 * 86400.0
            log_time_since = np.log1p(time_since_last)

            acts = seq_ts[i, :L]
            cur_t = float(cur_ts[i])

            cnt_1h = np.sum((cur_t - acts) <= 3600)
            cnt_6h = np.sum((cur_t - acts) <= 21600)
            cnt_1d = np.sum((cur_t - acts) <= 86400)
            cnt_7d = np.sum((cur_t - acts) <= 604800)

            log_cnt_1h = np.log1p(cnt_1h)
            log_cnt_6h = np.log1p(cnt_6h)
            log_cnt_1d = np.log1p(cnt_1d)
            log_cnt_7d = np.log1p(cnt_7d)

            if cnt_1d > 0:
                diffs_1d = cur_t - acts[(cur_t - acts) <= 86400]
                freshness_ratio_1d = np.mean(np.exp(-diffs_1d / 86400))
            else:
                freshness_ratio_1d = 0.0

            if cnt_7d > 0:
                diffs_7d = cur_t - acts[(cur_t - acts) <= 604800]
                freshness_ratio_7d = np.mean(np.exp(-diffs_7d / 604800))
            else:
                freshness_ratio_7d = 0.0

            exp_decay_1h = np.exp(-cnt_1h * 0.5) if cnt_1h > 0 else 1.0
            exp_decay_1d = np.exp(-cnt_1d * 0.1) if cnt_1d > 0 else 1.0
            exp_decay_7d = np.exp(-cnt_7d * 0.01) if cnt_7d > 0 else 1.0
    ...
    def _compute_cross_domain_features(
        self,
        seq_lens: Dict[str, np.ndarray],
        timestamps: np.ndarray,
        seq_ts: Dict[str, np.ndarray],
    ) -> np.ndarray:
        """Compute 4 cross-domain aggregated features:
        [active_domain_count, total_seq_len_log, max_recent_count_1d, mean_log_time_since_last]
        """
        ...
        for i in range(B):
            ...
            max_cnt_1d = 0
            log_time_since_list = []
            cur_t = float(timestamps[i])

            for domain in self.seq_domains:
                L = int(seq_lens[domain][i])
                if L == 0:
                    continue
                active_count += 1
                total_seq_len += L

                domain_ts = seq_ts[domain][i, :L]
                cnt_1d = np.sum((cur_t - domain_ts) <= 86400)
                if cnt_1d > max_cnt_1d:
                    max_cnt_1d = cnt_1d

                if L > 0:
                    last_ts = int(domain_ts[L - 1])
                    time_since_last = cur_t - last_ts if last_ts > 0 else 365 * 86400.0
                    log_time_since_list.append(np.log1p(time_since_last))

            out[i, 0] = float(active_count)
            out[i, 1] = np.log1p(total_seq_len)
            out[i, 2] = float(max_cnt_1d)

            if log_time_since_list:
                out[i, 3] = np.mean(log_time_since_list)
            else:
                out[i, 3] = np.log1p(365 * 86400.0)
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        Args:
            ...
            use_time_dense: if True, compute extra time-dense features (58-dim)
                and append to user_dense_feats.
            rank/world_size: DDP rank sharding over Row Groups. This dataset is
                an IterableDataset, so sharding must happen here rather than via
                DistributedSampler.
        """
        super().__init__()
        self.rank = int(rank)
        ...
        self.use_time_dense = use_time_dense
        ...
        if self.world_size > 1:
            self._rg_list = [
                rg for i, rg in enumerate(self._rg_list)
        ...
        # Record base dimension before adding time_dense reserved slots
        self._base_user_dense_dim = self.user_dense_schema.total_dim
        self._time_dense_offset: Optional[int] = None

        # Sync user_dense_schema dimension with time_dense features
        if self.use_time_dense:
            self._time_dense_offset = self.user_dense_schema.total_dim
            self.user_dense_schema.add(900001, EXTRA_TIME_DENSE_DIM)
        ...
        # ---- Initialize time dense feature computer ----
        if self.use_time_dense:
            self._time_dense_computer = TimeDenseFeaturesComputer(self.seq_domains)
        else:
            self._time_dense_computer = None
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        for domain in self.seq_domains:
            ...
            if ts_ci is not None:
                result[f'{domain}_real_ts'] = torch.from_numpy(ts_padded.copy())

        if self.use_time_dense and self._time_dense_computer is not None:
            seq_lens = {}
            seq_ts = {}
            for domain in self.seq_domains:
                seq_lens[domain] = result[f'{domain}_len'].numpy()
                real_ts_key = f'{domain}_real_ts'
                if real_ts_key in result:
                    seq_ts[domain] = result[real_ts_key].numpy()
                else:
                    seq_ts[domain] = np.zeros_like(result[f'{domain}_len'].numpy())

            time_dense_feats = self._time_dense_computer.compute(
                B, timestamps, None, seq_lens, seq_ts)

            # Write time_dense into the reserved 58-dim slots (in-place)
            user_dense_arr = result['user_dense_feats'].numpy()
            start = self._time_dense_offset  # type: ignore[index]
            end = start + EXTRA_TIME_DENSE_DIM
            user_dense_arr[:, start:end] = time_dense_feats
            result['user_dense_feats'] = torch.from_numpy(user_dense_arr)
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    logging.info(f"Parquet train: {train_rows} rows, valid: {valid_rows} rows, "
                 ...
                 f"use_time_dense={use_time_dense} ({EXTRA_TIME_DENSE_DIM} dims), "
                 f"rank={rank}, world_size={world_size}, "
                 f"rank_train_rows={train_dataset.num_rows}")
